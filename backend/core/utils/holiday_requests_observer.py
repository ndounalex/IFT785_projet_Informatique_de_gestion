...
class FrontEndNotifier(HolidayRequestObserver):
    ...
    def update(self, holiday_request, event: str):
        ...
        print(f"[Notification] Demande de congé #{holiday_request.id} faite par {holiday_request.owner} a été {event}.")
...
class EmailNotifier(HolidayRequestObserver):
    ...
    def update(self, holiday_request, event: str):
        print(f"[Email] À {holiday_request.owner} : votre demande a été {event.lower()}.")  # Simulé
